...
def DLiteMain(global_map, gx, gy, sx, sy):
    print(gx, gy, sx, sy)
    nodes_x, nodes_y = [], []
    ...
    while np.sum(np.abs(node.start - node.goal)) != 0:
        ...
        nodes_x.append(node.start[0])
        nodes_y.append(node.start[1])
...
def maze(width, height, complexity=.03, density=.008):
    # Only odd shapes
...
if __name__ == "__main__":
    ...
    nodes_x, nodes_y = DLiteMain(global_map, gx, gy, sx, sy)
